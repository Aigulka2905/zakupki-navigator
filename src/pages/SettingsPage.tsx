...
const integrations = [
  { name: "Сбербанк-АСТ", status: "connected", description: "ЭТП — синхронизация закупок" },
  { name: "ЕИС zakupki.gov.ru", status: "connected", description: "Реестр закупок 223-ФЗ" },
  { name: "РТС-тендер", status: "disconnected", description: "Электронная торговая площадка" },
  { name: "Росэлторг", status: "disconnected", description: "Электронная торговая площадка" },
];
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <Card className="p-6">
          <div className="mb-4 flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold text-foreground">Интеграции с ЭТП</h2>
          </div>
            ...
            {integrations.map((i) => (
              ...
              >
                ...
                <Button variant={i.status === "connected" ? "outline" : "default"} size="sm">
                  {i.status === "connected" ? "Отключить" : "Подключить"}
                </Button>
              </div>
            ))}
          </div>
